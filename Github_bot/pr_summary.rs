...
async fn handler(event: Result<WebhookEvent, serde_json::Error>) {
    ...
    let repo_context = env::var("repo_context").unwrap_or("This is a general software repository.".to_string());
    ...
    let system = &format!("You are an experienced software developer. You will act as a reviewer for a GitHub Pull Request titled \"{}\" in a repository with the following context: {}. Please be as concise as possible while being accurate and considering the repository's specific domain.", title, repo_context);
    ...
    for (_i, commit) in commits.iter().enumerate() {
        ...
        let question = format!(
            "The following is a GitHub patch for a repository with this context: {}. Please summarize the key changes in concise points, considering the repository's domain. Start with the most important findings, potential issues, or improvements.\n\n{}",
            repo_context,
    ...
    if reviews.len() > 1 {
        ...
        let question = format!(
            "Here is a set of summaries for source code patches in this PR for a repository with the following context: {}. Each summary starts with a ------ line. Write an overall summary. Present the potential issues, errors, and suggested improvements first, followed by the most important findings, in your summary. Consider the repository's specific domain in your analysis.\n\n{}",
            repo_context,
            &reviews_text
        );
        match lf.chat_completion(&chat_id, &question, &co).await {
